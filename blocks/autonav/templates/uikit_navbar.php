...
echo '  <ul class="uk-navbar-nav">'; //opens the top-level menu
...
foreach ($navItems as $ni) {
    ...
    if($ni->hasSubmenu && $ni->level>=1){                
        echo '<li class="uk-parent ' . $ni->classes . '" data-uk-dropdown role="presentation">'; //opens a nav item
    }else{
    ...
    if ($ni->hasSubmenu && $ni->level>=1) {        
        echo '<a href="#" role="button">' . $ni->name . ' <b class="caret"></b></a>'; //opens a dropdown sub-menu
        echo '<div class="uk-dropdown uk-dropdown-navbar">';
        echo '<ul class="uk-nav uk-nav-navbar" role="menu">'; //opens a dropdown sub-menu
    } else {        
